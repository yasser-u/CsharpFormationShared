...
public class HumanPlayer : Player
{
    ...
    public override async Task<Result<PlayerMove>> GetNextMove()
    {
        ...
        {
            return Result.Failure<PlayerMove>("Invalid target cell row must be between 1 and 3");
        }

        if (int.TryParse(splittedInput?[1], out int targetColumn) is false ||
            targetColumn < 1 || targetColumn > 3)
        {
            return Result.Failure<PlayerMove>("Invalid target cell column must be between 1 and 3");
        }
